...
const DropDowns = ({setsameplant}) => {
  ...
  localStorage.setItem('PlantV', value);
  ...
  const onChangeSite = (valueSelected) => {
    ...
    //onchange runs when select value is changed by user with parameter 'value' of option selected by user as 'valueSelected'
    setSiteValue(valueSelected);//get the value which is selected by user
    // options.map((loopThroughOptions) => loopThroughOptions.value == valueSelected ? setId(loopThroughOptions.inputDisplay) : setId(options[0].inputDisplay))//now this will get the id of value which is selected by user
  }
