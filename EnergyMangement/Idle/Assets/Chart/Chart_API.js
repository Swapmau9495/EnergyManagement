...
function Chart_API({Refresh,AssetNames,energyassettrendFetchHelper,setRefreshdisable}) {
  const [activeFilter] = useActiveFilter();
  const {  plant,start_date,end_date } = activeFilter;
  const {response, isloading, error, execute} = energyassettrendFetchHelper;
  useEffect(()=>{
    if(isloading){
  ...
  useEffect(() => {
    if (plant.value ){
      execute({ url: `_ems/asset/_trend/_all`,  data: {
                   from:start_date,
                   to:end_date,
  ...
  let Data;
  if (!error) {
    // Data = isloading ? Loader : <Chart AssetNames={AssetNames} data={response} />;
    if(response){
      Data = isloading ? Loader : (<Chart  AssetNames={AssetNames} data={response} />);
    } else if(!response){
  ...
  return (
    <EuiFlexItem>
      <EuiPanel style={{ height: "350px" }} >
        <EuiTitle size="l">
          <EuiText>
            <strong>Idle Energy by Asset</strong>
          </EuiText>
        </EuiTitle>
        <EuiSpacer/>
        {Data}
